...
CONF_THRESH = 0.5
IOU_THRESHOLD = 0.4
def publish_coordinates(centerx,centery):
    
    pub = rospy.Publisher('coordinates', Point, queue_size=0)
    ...
    def infer_warmup(self, raw_image_generator):
        ...
        for i in range(self.batch_size):
            ...
            for j in range(len(result_boxes)):
                box = result_boxes[j]
                centerx = result_boxes[j][0]+((result_boxes[j][2]-result_boxes[j][0])/2)
                centery = result_boxes[j][1]+((result_boxes[j][3]-result_boxes[j][1])/2)
                try:
                    publish_coordinates(centerx,centery)
